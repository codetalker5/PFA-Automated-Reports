q1a = """
    ...
    total as (
            ...
            where bl_program_id = 'OD009202400'
            group by bl_subject
...
q1c = """
    WITH bl AS (
        ...
        WHERE bl_student_level = 5 and bl_program_id = 'OD009202400'
        GROUP BY bl_subject
    ),
    el AS (
        SELECT 
        ...
        FROM analytics.der_offline_baseline_endline_analysis dobea 
        WHERE el_student_level = 5 and bl_program_id = 'OD009202400'
        GROUP BY el_subject
    )
    SELECT 
        b.bl_subject,
...
q1d = """
    with nos as (
        select bl_subject, COUNT(distinct bl_student_id) as pct from analytics.der_offline_baseline_endline_analysis dobea 
        where bl_program_id = 'OD009202400'
        group by bl_subject 
    ) SELECT * from nos
    """
